...
- `requests` - HTTP requests for optional threat intelligence APIs
- `reportlab` - PDF generation fallback
- `jinja2` - HTML template rendering for PDF export
- `weasyprint` - HTML to PDF conversion
- `torchvision` - Runtime compatibility dependency for transformer stack

**Streamlit Cloud Deployment**
If deploying to Streamlit Cloud, include `packages.txt` with required system libraries for WeasyPrint:
```text
libpango-1.0-0
libpangoft2-1.0-0
```
